...
def load_data_for_2026_season():
    ...
    try:
        ...
        if 'season_type' in df_players_2026.columns:
            df_players_2026 = df_players_2026[df_players_2026['season_type'] == 'REG'].copy()
    except Exception:
...
def calculate_2026_defense_by_position(df_players_2026):
    ...
    games_per_team = (
        df_players_2026.groupby('opponent_team')['week']
        .nunique()
        .reset_index()
        .rename(columns={'week': 'games_played_2026'})
    )

    def_2026 = df_players_2026.groupby(['opponent_team', 'position']).agg(
        rec_yds_allowed_2026=('receiving_yards', 'sum'),
        rush_yds_allowed_2026=('rushing_yards', 'sum'),
        pass_yds_allowed_2026=('passing_yards', 'sum')
    ).reset_index()

    def_2026 = pd.merge(def_2026, games_per_team, on='opponent_team', how='left')

    def_2026['rec_yds_allowed_pg_2026'] = def_2026['rec_yds_allowed_2026'] / def_2026['games_played_2026']
    def_2026['rush_yds_allowed_pg_2026'] = def_2026['rush_yds_allowed_2026'] / def_2026['games_played_2026']
    def_2026['pass_yds_allowed_pg_2026'] = def_2026['pass_yds_allowed_2026'] / def_2026['games_played_2026']

    def_2026['rec_def_rank_2026'] = def_2026.groupby('position')['rec_yds_allowed_pg_2026'].rank(ascending=True)
    def_2026['rush_def_rank_2026'] = def_2026.groupby('position')['rush_yds_allowed_pg_2026'].rank(ascending=True)
